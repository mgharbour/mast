...
package body Mast_Editor.Event_Handlers is
   ...
   begin
      ...
      if (Get_Active_Text (SEH_Dialog.Seh_Type_Combo) =
            "Activity")
      then
         Han_Ref := new Activity;
         if Han_Ref_Old /=null and then
           Han_Ref_Old.all in
         ...
         if (Get_Active_Text (SEH_Dialog.Ser_Combo) /= "(NONE)") then
            Serv_Name  :=
              To_Var_String (Get_Active_Text (SEH_Dialog.Ser_Combo));
            Serv_Index :=
         ...
         if (Get_Active_Text (SEH_Dialog.Op_Combo) /= "(NONE)") then
            Op_Name  :=
              To_Var_String (Get_Active_Text (SEH_Dialog.Op_Combo));
            Op_Index :=
      ...
      elsif (Get_Active_Text (SEH_Dialog.Seh_Type_Combo) =
               "System Timed Activity")
      then
         Han_Ref :=  new System_Timed_Activity;
         if Han_Ref_Old /=null and then
           Han_Ref_Old.all in
           Mast.Graphs.Event_Handlers.Simple_Event_Handler'Class
         then
         ...
         Set_Rate_Factor
           (Rate_Divisor (Han_Ref.all),
            Positive'Value (Get_Text (SEH_Dialog.Rate_Entry)));
               ...
               "Delay")
      then
         Han_Ref :=  new Delay_Event_Handler;
         if Han_Ref_Old /=null and then
           Han_Ref_Old.all in
           Mast.Graphs.Event_Handlers.Simple_Event_Handler'Class
         then
         ...
         Set_Delay_Min_Interval
           (Delay_Event_Handler (Han_Ref.all),
            Time'Value (Get_Text (SEH_Dialog.Delay_Min_Entry)));
               ...
               "Offset")
      then
         Han_Ref := new Offset_Event_Handler;
         if Han_Ref_Old /=null and then
           Han_Ref_Old.all in
           Mast.Graphs.Event_Handlers.Simple_Event_Handler'Class
         then
         ...
         Eve_Name :=
           To_Var_String
           (Get_Active_Text (SEH_Dialog.Ref_Event_Combo));
         if Item.ME_Tran.Tran /= null
           and then (Get_Active_Text (SEH_Dialog.Ref_Event_Combo)) /=
           "(NONE)"
         then
            Eve_Ref :=
   ...
   is
      ...
      Iter : Mast.Graphs.Event_Handlers.Iteration_Object;
      The_Link : Mast.Graphs.Link_Ref;
   begin
      Change_Control.Changes_Made;
      if (Get_Active_Text (MIEH_Dialog.Minput_Type_Combo) =
            "Concentrator")
      then
         Han_Ref := new Concentrator;
      ...
      elsif (Get_Active_Text (MIEH_Dialog.Minput_Type_Combo) =
               "Barrier")
      then
         Han_Ref := new Barrier;
         if Han_Ref_Old /=null and then
   ...
   begin
      Change_Control.Changes_Made;
      if (Get_Active_Text (MOEH_Dialog.Moutput_Type_Combo) =
            "Delivery Server")
      then
         Han_Ref                                                       :=
           new Delivery_Server;
         ...
         then
            ...
            Mast.Graphs.Event_Handlers.Rewind_Output_Links
              (Mast.Graphs.Event_Handlers.Output_Event_Handler(Han_Ref_Old.all),
               Iter);
         ...
         Set_Policy
           (Delivery_Server (Han_Ref.all),
            Delivery_Policy'Value
              (Get_Active_Text (MOEH_Dialog.Del_Policy_Combo)));
      elsif (Get_Active_Text (MOEH_Dialog.Moutput_Type_Combo) =
               "Query Server")
      then
         Han_Ref                                                       :=
           new Query_Server;
         ...
         Set_Policy
           (Query_Server (Han_Ref.all),
            Request_Policy'Value
              (Get_Active_Text (MOEH_Dialog.Que_Policy_Combo)));
      elsif (Get_Active_Text (MOEH_Dialog.Moutput_Type_Combo) =
               "Multicast")
      then
         Han_Ref := new Multicast;
         if Han_Ref_Old /=null and then
           Han_Ref_Old.all in
           Mast.Graphs.Event_Handlers.Output_Event_Handler'Class
         then
   ...
   begin
      ...
      loop
         ...
         String_List.Append (Ref_Event_Combo_Items, To_String (Eve_Name));
      end loop;
      ...
      if Han_Ref /= null then
         if Han_Ref.all'Tag = Mast.Graphs.Event_Handlers.Activity'Tag then
            Set_Text_In_Combo_Box (SEH_Dialog.Seh_Type_Combo, "Activity");
            ...
            then
               ...
               Set_Text_In_Combo_Box (SEH_Dialog.Op_Combo, "(NONE)");
            end if;
            if Mast.Graphs.Event_Handlers.Activity_Server
              (Activity (Han_Ref.all)) /=
              null
            then
               ...
               Set_Text_In_Combo_Box (SEH_Dialog.Ser_Combo, "(NONE)");
            end if;
         ...
         then
            Set_Text_In_Combo_Box (SEH_Dialog.Seh_Type_Combo, "Rate Divisor");
            Set_Text
         ...
         then
            Set_Text_In_Combo_Box (SEH_Dialog.Seh_Type_Combo, "Delay");
            Set_Text
         ...
         then
            Set_Text_In_Combo_Box (SEH_Dialog.Seh_Type_Combo, "Offset");
            Set_Text
              (SEH_Dialog.Delay_Max_Entry,
               Time_Image
                 (Mast.Graphs.Event_Handlers.Delay_Max_Interval
            ...
            then
               ...
               Set_Text_In_Combo_Box (SEH_Dialog.Ref_Event_Combo, "(NONE)");
            end if;
            Show (SEH_Dialog.Table1);
            Show (SEH_Dialog.Delay_Table);
   ...
   begin
      ...
      if Han_Ref /= null then
         ...
         then
            ...
            Set_Text_In_Combo_Box (MIEH_Dialog.Minput_Type_Combo, "Barrier");
         else
            null;
         end if;
      end if;
   ...
   begin
      ...
      if Han_Ref /= null then
         if Han_Ref.all'Tag = Mast.Graphs.Event_Handlers.Activity'Tag then
	    Set_Text (Layout,"Activity");
	    Cairo.Move_To (Cr, 0.0, 2.0);
	    Pango.Cairo.Show_Layout (Cr, Layout);
	    
            if Mast.Graphs.Event_Handlers.Activity_Server
              (Activity (Han_Ref.all)) /= null
            then
		 Text:=Text&"Op:" &
		    Cut (To_String
			   (Name
			      (Mast.Graphs.Event_Handlers.Activity_Operation
				 (Activity (Han_Ref.all)))), 20)&NL;
            else
		 Text:=Text&"Op: (NOT ASSIGNED)"&NL;
	    end if;
		 ...
		 Text:=Text&"Serv:" &
                  Cut (To_String
                       (Name
                        (Mast.Graphs.Event_Handlers.Activity_Server
                         (Activity (Han_Ref.all)))), 18);
            else
	       Text:=Text&"Serv: (NOT ASSIGNED)";
            end if;
         elsif Han_Ref.all'Tag =
           Mast.Graphs.Event_Handlers.System_Timed_Activity'Tag
         then
	    Set_Text (Layout,"System Timed Activity");
	    Cairo.Move_To (Cr, 0.0, 2.0);
	    Pango.Cairo.Show_Layout (Cr, Layout);
	       ...
	       Text:=Text&"Op:" &
                  Cut (To_String
                       (Name (Mast.Graphs.Event_Handlers.Activity_Operation
                              (Activity (Han_Ref.all)))) , 20)&NL;
            else
	       Text:=Text&"Op: (NOT ASSIGNED)"&NL;
            end if;
	    ...
	    Text:=Text&"Rate Factor:" & Positive'Image
               (Mast.Graphs.Event_Handlers.Rate_Factor
                (Rate_Divisor (Han_Ref.all)));
         elsif Han_Ref.all'Tag =
           Mast.Graphs.Event_Handlers.Delay_Event_Handler'Tag
	    ...
	    Text:=Text&"Max interval:" & Time_Image
               (Mast.Graphs.Event_Handlers.Delay_Max_Interval
                (Delay_Event_Handler (Han_Ref.all)))&NL&
	      "Min Interval:" & Time_Image
		 (Mast.Graphs.Event_Handlers.Delay_Min_Interval
		    (Delay_Event_Handler (Han_Ref.all)));
         elsif Han_Ref.all'Tag =
           Mast.Graphs.Event_Handlers.Offset_Event_Handler'Tag
   ...
   begin
      ...
      if Han_Ref /= null then
         ...
         then
	    Set_Text 
	      (Layout,"Delivery Server");
	    Cairo.Move_To (Cr, 0.0,2.0);
	    Pango.Cairo.Show_Layout (Cr, Layout);
	    
	    Text:=To_Unbounded_String
	      ("Delivery Policy:" & Delivery_Policy'Image
                 (Policy (Delivery_Server (Han_Ref.all))));
         elsif Han_Ref.all'Tag =
           Mast.Graphs.Event_Handlers.Query_Server'Tag
         then
	    ...
	    Text:=To_Unbounded_String
	      ("Req Policy: " & Request_Policy'Image
                 (Policy (Query_Server (Han_Ref.all))));
         elsif Han_Ref.all'Tag =
           Mast.Graphs.Event_Handlers.Multicast'Tag
         then
	    Set_Text 
   ...
   begin
      if ((Get_Active_Text (SEH_Dialog.Seh_Type_Combo) = "Activity") or
            (Get_Active_Text (SEH_Dialog.Seh_Type_Combo) =
               "System Timed Activity"))
        and then ((Get_Active_Text (SEH_Dialog.Ser_Combo) = "(NONE)") or
                    (Get_Active_Text (SEH_Dialog.Op_Combo) = "(NONE)"))
      then
         Gtk_New (Editor_Error_Window, Gtk_Window(SEH_Dialog));
         Set_Text
           (Editor_Error_Window.Label,
   ...
   begin
      ...
      Gtk_New (SEH_Dialog, Gtk_Window(Res.Dialog));
   ...
   begin
      ...
      Gtk_New (MIEH_Dialog, Gtk_Window(Res.Dialog));
   ...
   begin
      ...
      Gtk_New (MOEH_Dialog, Gtk_Window(Res.Dialog));
   ...
   is
      ...
      Me_Data    : Me_Event_Handler_And_Dialog_Ref :=
        new Me_Event_Handler_And_Dialog;
   begin
      Gtk_New (SEH_Dialog, Gtk_Window(Item.ME_Tran.Dialog));
      Read_Parameters (Item, Gtk_Dialog (SEH_Dialog));
      Me_Data.It  := Item;
      Me_Data.Dia := Gtk_Dialog (SEH_Dialog);
   ...
   is
      ...
      Me_Data     : Me_Event_Handler_And_Dialog_Ref :=
        new Me_Event_Handler_And_Dialog;
   begin
      Gtk_New (MIEH_Dialog, Gtk_Window(Item.ME_Tran.Dialog));
      Read_Parameters (Item, Gtk_Dialog (MIEH_Dialog));
      Me_Data.It  := Item;
      Me_Data.Dia := Gtk_Dialog (MIEH_Dialog);
   ...
   is
      ...
      Me_Data     : Me_Event_Handler_And_Dialog_Ref :=
        new Me_Event_Handler_And_Dialog;
   begin
      Gtk_New (MOEH_Dialog, Gtk_Window(Item.ME_Tran.Dialog));
      Read_Parameters (Item, Gtk_Dialog (MOEH_Dialog));
      Me_Data.It  := Item;
      Me_Data.Dia := Gtk_Dialog (MOEH_Dialog);
   ...
   begin
      Lin_Name :=
        To_Var_String (Get_Active_Text (Add_Link_Dialog.Combo));
      if (Item.ME_Tran.Tran /= null)
        and then ((Get_Active_Text (Add_Link_Dialog.Combo)) /= "")
      ...
      loop
         Mast.Transactions.Get_Next_Internal_Event_Link
           (Item.ME_Tran.Tran.all,
            ...
            Eve_Index);
         Eve_Name := Mast.Graphs.Name (Eve_Ref.all);
         String_List.Append (Combo_Items, To_String (Eve_Name));
      end loop;

      Gtk_New (Add_Link_Dialog, Gtk_Window(Item.ME_Tran.Dialog));
