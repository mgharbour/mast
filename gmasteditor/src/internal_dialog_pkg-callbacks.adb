...
package body Internal_Dialog_Pkg.Callbacks is
      ...
      if Iter /= Null_Iter then
         Set
           (Internal_Dialog.Tree_Store,
            ...
            Get_Active_Text (Select_Ref_Event_Dialog.Ref_Event_Combo));
      end if;
	 ...
	 Get_Coords(Event,X_Win,Y_Win);
         ...
         if Column = Object.Ref_Event_Col then
            ...
            then
               ...
               Gtk_New (Select_Ref_Event_Dialog, Gtk_Window(Object));
         ...
         if N_Children (Tree_Store, Null_Iter) = Gint (1) then
            ...
            exception
               when Event_Not_Found =>
                  Gtk_New 
		    (Editor_Error_Window, Gtk_Window(Item.ME_Tran.Dialog));
                  Set_Text
               ...
               exception
                  when Event_Not_Found =>
                     if not Error_Window_Created then
                        Gtk_New 
			  (Editor_Error_Window, 
			   Gtk_Window(Item.ME_Tran.Dialog));
   ...
   begin
      if (Get_Active_Text (Select_Req_Type_Dialog.Combo) =
            "Hard Local Deadline")
      then
         Req_Ref := new Hard_Local_Deadline;
      elsif (Get_Active_Text (Select_Req_Type_Dialog.Combo) =
               "Soft Local Deadline")
      then
         Req_Ref := new Soft_Local_Deadline;
      elsif (Get_Active_Text (Select_Req_Type_Dialog.Combo) =
   ...
   begin
      ...
      Gtk_New (Select_Req_Type_Dialog, Gtk_Window(Object));
