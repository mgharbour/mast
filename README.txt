...
*The holistic analysis now supports multipath transactions

2. BINARY INSTALLATION
----------------------

    Windows (tested on Windows 10): 
      - unzip the file mast-bin-win-1-5-1-x.zip into a directory of your
        choice.
      - This causes the following files and directories to appear:
            ...
            gmastresults.bat      : script for invoking the results viewer
            gmasteditor.exe       : graphical editor for MAST models (alpha)
            gmasteditor.bat       : script for invoking the graphical editor
      ...
      - put both the MAST directory and the bin directory contained in
        ...
        the tools; you may create links (shortcuts) to the gmast.bat
        script and similar scripts in the desktop or another
        comfortable place.
         
    Linux (tested on Ubuntu 18.04):
      - requires a pdf reader for the documentation, and gtk-3.x
        (available in the Ubuntu distribution) for the GUI.
      - unzip and extract the files in mast-bin-linux-1-5-1-x.tar.gz into
        the directory of your choice:
            tar zxvf mast-bin-linux-1-5-1-x.tar.gz
...
----------------------
      ...
      - Requires the gnat compiler (libre.adacore.com). We have used
        the Community 2019 version of gnat in Linux, and in Windows.
      ...
      - The programs with graphical user interface (gmast_analysis,
        gmastresults, gmasteditor, pr_editor) require an external
        installation of gtkada. We have performed our tests with the
        2019 version.

      - The programs in mast_xml require the xmlada library but this
        now comes by default with the compiler so it does not need a
        special installation.

      - Compiling on Linux
	
        The script called compile can be called to compile all the
        mast programs. It calls a script inside the directory of each
        program.

        To compile each program we provide compile scripts using
        project files (*.gpr) which encapsulate the code
        dependencies. For instance to compile the gmast graphical
        editor in gmast/src, the compile script contains:
      
             gnatmake -P gmast_analysis.gpr
      ...
      - Compiling in Windows
        ...
        To compile in Windows we provide a compile.bat script that
        compiles all the mast programs assuming that gtkada is
        installed.
	
        After compilation set the appropriate PATH, or move the
        executable program to a directory in the search PATH.
...
------------------------------
   ...
   -----------------
      ...
      output_file: will contain the results of the analysis (text, XML or XMI)
