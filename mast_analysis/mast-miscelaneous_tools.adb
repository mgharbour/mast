...
with Mast.Tool_Exceptions,Mast.Shared_Resources,
  ...
  Ada.Text_IO, Var_Strings, Trimmed_Image;
...
package body Mast.Miscelaneous_Tools is
   ...
   is
      ...
      is
         ...
         procedure Process_Critical_Sections
           (The_Operation_Ref : Operations.Operation_Ref;
   ...
   begin
      if Restrictions.Restricted_Multipath_Transactions_Only 
        (The_System,False) 
      then
         Translation.Translate_Linear_System(The_System,Transaction,False);
      else
         Set_Exception_Message
           ("Calculate_Blocking_Times not yet implemented for"&
              " Multipath transaction systems");
         raise Tool_Exceptions.Tool_Failure;
      end if;
   ...
   function Check_System_Schedulability
     (The_System : Mast.Systems.System;
      Verbose : Boolean:=False) return Boolean is
   ...
   begin
      ...
      loop
         ...
         if Graphs.Has_Timing_Requirements(A_Link_Ref.all) and then
           Graphs.Has_Results(A_Link_Ref.all) 
         then
   ...
   is
      ...
      begin
         ...
         if Natural(Proc_Number) > Mast.Processing_Resources.Lists.Size
           (The_System.Processing_Resources)
         then
            Set_Exception_Message("Processor number too high");
   ...
   begin
      if Mast.Restrictions.Restricted_Multipath_Transactions_Only 
        (The_System,False)
      then
         ...
         if Verbose then
            Put_Line("Calculate_Processing_Resource_Utilization"&
                       ...
                       " Unrestricted Multipath Transaction systems");
         end if;
         Tool_Exceptions.Set_Tool_Failure_Message
           ("Calculate_Processing_Resource_Utilization"&
              " not yet implemented for"&
   ...
   is
      ...
      package Translation is new Mast.
        Linear_Translation (Processor_ID_Type,
                            Transaction_ID_Type,
                            Task_ID_Type,
                            Max_Processors,
                            Max_Transactions,
   ...
   begin
      ...
      Translation.Translate_Linear_System_With_Results(The_System,
                                                       My_System,
                                                       False);
      -- I := 1;
      -- NT := 0;
      ...
      for I in 1..Max_Transactions loop
         Average_R := Average_R + My_System(I).The_Task(My_System(I).Ni).Rij;
      end loop;

      return Average_R/Time(Max_Transactions);

   end Calculate_Average_Response_Times;

   ------------------------------------------
   -- Write End-to-End Deadlines into file --
   ------------------------------------------
   ...
   begin
      ...
      Put(File,"ED ");
      for I in 1..Max_Transactions loop
         Ada.Text_IO.Put(File,Img(My_System(I).The_Task(My_System(I).Ni).Dij)&" ");
      end loop;

   end Write_ED_to_file;
   ...
   begin
      ...
      Translation.Translate_Linear_System_With_Results(The_System,
                                                       My_System,
                                                       False);
      Translation.Show_Linear_Translation(My_System);

      Put_Line("final Scheduling Parameter Overview");
      Translation.Show_Scheduling_Parameters(My_System);

   end Print_System_Overview;
