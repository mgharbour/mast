...
package body Mast.Graphs is
   pragma Warnings (Off, "use clause for type * defined at mast-events.ads:62 has no effect", Reason => "TBD");

   use type Events.Event_Ref;
   pragma Warnings (On, "use clause for type * defined at mast-events.ads:62 has no effect");
   ...
   begin
      ...
      for I in 1..Event_Handler_Lists.Size(The_List) loop
         Event_Handler_Lists.Get_Next_Item(Res_Ref,The_List,Iterator);
         Print_XMI(File,Res_Ref.all,Indentation,False);
      end loop;
   end Print_XMI;
   ...
   begin
      ...
      for I in 1..Link_Lists.Size(The_List) loop
         Link_Lists.Get_Next_Item(Res_Ref,The_List,Iterator);
         if Has_Results(Res_Ref.all) then
            Print_XMI_Results(File,Res_Ref.all,Indentation,False);
         end if;
      end loop;
