...
with Mast.Processing_Resources,
  Mast.Annealing_Parameters, Mast.Linear_Translation, Priority_Queues,
  Hash_Lists, List_Exceptions, Mast.Tools.Schedulability_Index,
  Mast.Scheduling_Parameters, Mast.Scheduling_Policies;

with Ada.Text_IO, Ada.Numerics.Discrete_Random, Ada.Numerics.Float_Random,
  Ada.Numerics.Generic_Elementary_Functions, Mast.Max_Numbers;
...
package body Mast.Linear_Priority_Assignment_Tools is
   ...
   is
      ...
      Audsley_Stop_Time  : constant Duration :=
      ...
      begin
         ...
         for I in Processor_ID_Type range 1 .. Max_Processors loop
            ...
            begin
               while not Deadlines_Lists.Empty (Lists_For_Processors (I)) loop
                  J := J + 1;
            ...
            while not VP_Lists.Empty (Lists_For_VP (I)) loop
               ...
               if Debug then
                  Put_Line
                    ("   Priority Assigned = " &
                     Priority'Image
                        (My_System (Item2.Transaction).The_Task (Item2.Action).
                    Prioij));
               end if;
      ...
      procedure Single_Priority_Mapping (P : Processor_ID_Type) is
      ...
      begin
         ...
         if not P_Lists.Empty (List_For_P) then
            ...
            Min_VP :=
              My_System (Item1.Transaction).The_Task (Item1.Action).Pav.
              Virtual_Prio;
         else
         ...
         while not VP_Lists.Empty (List_For_VP) loop
      ...
      begin
         ...
         for K in Processor_ID_Type range 1 .. Max_Processors loop
            if Optimization_In_Processor (K) then
               declare
                  type Lists_Of_Task is
                    array (Integer range 1 .. Queue_Length (K))
                           of Deadline_Element;
               ...
               begin
                  ...
                  loop
                     exit when My_System (I).Ni = 0;
                     for J in 1 .. My_System (I).Ni loop
                        if (My_System (I).The_Task (J).Procij = K)
                          and then not My_System (I).The_Task (J).Pav.
                             Hard_Prio
                          and then not My_System (I).The_Task (J).Pav.
                             Is_Polling
                        then
                  ...
                  if Debug then
                     ...
                     for I in 1 .. Num_Of_Task loop
                        ...
                        Put
                          ("task : " &
                           Task_ID_Type'Image (List_Of_Task (I).Action));
                        Put
                          ("   deadline = " &
                           Time'Image
                              (
                          My_System (List_Of_Task (I).Transaction).The_Task (
                          List_Of_Task (I).Action).Dij));
                        Put_Line
                          ("   prioridad = " &
                           Priority'Image
                              (
                          My_System (List_Of_Task (I).Transaction).The_Task (
                          List_Of_Task (I).Action).Prioij));
                     end loop;
                     New_Line;
                  end if;

                  -- Order the task
                  for I in 2 .. (Num_Of_Task) loop
                     for J in reverse I .. (Num_Of_Task) loop
                        if My_System (List_Of_Task (J).Transaction).The_Task (
                             List_Of_Task (J).Action).Prioij >
                  ...
                  for I in reverse 2 .. Num_Of_Task loop
                     Failed := True;
                     for J in reverse 1 .. I loop
                        ...
                        My_System (List_Of_Task (I).Transaction).The_Task (
                          List_Of_Task (I).Action).Prioij :=
                          My_System (List_Of_Task (J).Transaction).The_Task (
                          List_Of_Task (J).Action).Prioij;
                        My_System (List_Of_Task (J).Transaction).The_Task (
                          List_Of_Task (J).Action).Prioij := Prio;

                        if Debug then
                           Put_Line ("Changing priorities.....");
                           for L in 1 .. Num_Of_Task loop
                              Put
                                ("trans : " &
                                 Transaction_ID_Type'Image
                              ...
                              Put
                                ("task : " &
                                 Task_ID_Type'Image (List_Of_Task (L).Action));
                              Put
                                ("   deadline = " &
                                 Time'Image
                                    (
                              ...
                              Put_Line
                                ("   prioridad = " &
                                 Priority'Image
                                    (
                                My_System (List_Of_Task (L).Transaction).
                                The_Task (List_Of_Task (L).Action).Prioij));
                           end loop;
                           New_Line;
                        end if;
      ...
      begin
         ...
         for I in Transaction_ID_Type range 1 .. Max_Transactions loop
            exit when My_System (I).Ni = 0;
            Max_D := Large_Time;
            for J in reverse 1 .. My_System (I).Ni loop
               ...
               then
                  ...
                  if My_System (I).The_Task (J).Pav.Calculate_Factor then
                     My_System (I).The_Task (J).Pav.Factor :=
                       (My_System (I).The_Task (J).Pav.D_0 - Assigned_D) /
                       Sum_C;
                     Sum_C                                 := 0.0;
                     Assigned_D                            :=
                       My_System (I).The_Task (J).Pav.D_0;
      ...
      procedure Toggle_Priorities (The_Neighbor : Neighbor_Type) is

         Temp_Priority : Virtual_Priority;

      begin
         Temp_Priority :=
           My_System (The_Neighbor.Action1.Transaction).The_Task (
           The_Neighbor.Action1.Action).Pav.Virtual_Prio;
         My_System (The_Neighbor.Action1.Transaction).The_Task (
           The_Neighbor.Action1.Action).Pav.Virtual_Prio :=
           My_System (The_Neighbor.Action2.Transaction).The_Task (
           The_Neighbor.Action2.Action).Pav.Virtual_Prio;
         My_System (The_Neighbor.Action2.Transaction).The_Task (
           The_Neighbor.Action2.Action).Pav.Virtual_Prio := Temp_Priority;

         Single_Priority_Mapping
           (My_System (The_Neighbor.Action1.Transaction).The_Task (
           The_Neighbor.Action1.Action).Procij);
      ...
      function Energy return Long_Float is
         ...
         Middle_Energy            : constant Long_Float :=
            Long_Float'Last / 2.0;
                  ...
                  if My_System (I).The_Task (J).Pav.Calculate_Factor then
                     ...
                     then
                        Energy_Plus := Energy_Plus +
                                       Long_Float (My_System (I).The_Task (J).
                          Rij -
                                                   My_System (I).The_Task (J).
                          Pav.D_I);
                     else
                        Energy_Less := Energy_Less +
                                       Long_Float (My_System (I).The_Task (J).
                          Rij -
                                                   My_System (I).The_Task (J).
                          Pav.D_I);
      ...
      begin
         ...
         loop
            Processor :=
              Processor_ID_Type ((Natural_Random.Random (Natural_Generator) mod
                                  Natural (Max_Processors)) +
                                 1);
            exit when List_For_Processor.Size
                         (Lists_For_Processors (Processor)) >=
                      2;
         end loop;

         -- Selects two different actions in the processing resource
         Action1 :=
           (Natural_Random.Random (Natural_Generator) mod
            List_For_Processor.Size (Lists_For_Processors (Processor))) +
           1;
         loop
            Action2 :=
              (Natural_Random.Random (Natural_Generator) mod
      ...
      begin
         ...
         if Sched_Index > Optimum_Sched_Index then
            ...
            if Verbose then
               Put
                 ("Saving optimum priority assignment with " &
      ...
      begin
         if Overall_Schedulable then
            if Verbose then
               Put
                 ("Restoring optimum priority assignment with " &
                  "schedulability index : ");
               Mast.Tools.Schedulability_Index.Print (Optimum_Sched_Index);
            end if;
            for I in Transaction_ID_Type range 1 .. Max_Transactions loop
      ...
      begin
         ...
         if Iters_With_Greater_Energy = Max_Iters_With_Greater_Energy then
            Random_Jumps := Random_Jumps + 1;
            if Random_Jumps = Max_Random_Jumps then
               return True;
            else
               for I in 1 .. Random_Jumps loop
            ...
            if (Float (Counter_Of_Aborted_Analysis) / Float (Iters_To_Stop)) >
               0.5
            then
               return True;
            end if;
   ...
   begin
      ...
      loop
         ...
         loop
            ...
            if Neighbor_Energy < Minimum_Energy then
               ...
               Iters_With_Greater_Energy       := Iters_With_Greater_Energy +
                                                  1;
            ...
            -- Remains the neighbor assignment of priorities
               ...
               if Long_Float_Functions.Exp (X) >=
                  Long_Float (Ada.Numerics.Float_Random.Random
                                 (Float_Generator))
               then
                  Actual_Energy := Neighbor_Energy;
               -- Remains the neighbor assignment of priorities
      ...
      if Verbose then
         ...
         Put_Line
           ("   Number of times the analysis was stopped ----------> " &
