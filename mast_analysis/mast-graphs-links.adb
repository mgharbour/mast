...
package body Mast.Graphs.Links is
   ...
   overriding function Has_Results
   ...
   overriding function Has_Timing_Requirements
     (The_Link : Regular_Link) return Boolean is
   begin
   ...
   overriding procedure Print_Results
     (File : Ada.Text_IO.File_Type;
      The_Link : in out Regular_Link;
      Indentation : Positive;
      Finalize    : Boolean:=False)
   ...
   overriding procedure Print_XML_Results
     (File : Ada.Text_IO.File_Type;
      The_Link : in out Regular_Link;
      Indentation : Positive;
      Finalize    : Boolean:=False)
   ...
   is
      ...
      Finish : Boolean;
   begin
      Has_Requirements := The_Link.Event.all in Mast.Events.Internal_Event 
   and then The_Link.Link_Timing_Requirements /= null;
      Finish := not Has_Requirements;
      if The_Link.Event /= null then
         Events.Print_XMI(File,The_Link.Event.all,Indentation,Finish);
      end if;
      if Has_Requirements then
         Mast.Timing_Requirements.Print_XMI
      (File,The_Link.Link_Timing_Requirements.all,
       Indentation+2,False);
         Ada.Text_IO.Set_Col(File, Ada.Text_IO.Count(Indentation));
   ...
   begin
      if The_Link.Link_Time_Results/=null then
         Results.Print_XMI(File,The_Link.Link_Time_Results.all,
                           Indentation,Finalize);
      end if;
