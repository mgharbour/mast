...
package Mast.Consistency_Checks is
   ...
   -- Rule 26: Periods are not null. This includes
   --            26:  external event periods,
   --            26a: minimum interarrival times for sporadic tasks,
   --            26b: ticker periods for timers,
   --            26c: polling periods for polling servers, and
   --            26d: replenishment periods for sporadic servers
   ...
   function Consistent_Network_Drivers
     (The_System : Mast.Systems.System;
      Verbose : Boolean := True) return Boolean;
   -- Checks the following rules for all network drivers in the system:
   --    1- There is at most one driver per processor-network pair
   --    2- Every driver has a scheduling server
   --    3- Every driver has a send and receive operation
