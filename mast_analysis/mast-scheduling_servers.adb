...
with Mast.Schedulers.Primary, Mast.Schedulers.Secondary,
  Mast.Scheduling_Policies, Mast.IO, Mast.XMI, 
...
package body Mast.Scheduling_Servers is
   ...
   pragma Warnings (Off, "use clause for private type * defined at var_strings.ads:39 has no effect", Reason => "TBD");
   use type Var_Strings.Var_String;
   pragma Warnings (On, "use clause for private type * defined at var_strings.ads:39 has no effect");
   ...
   procedure Adjust
     (The_List : Lists.List;
      Scheduler_List : Schedulers.Lists.List)
   is
      Iterator : Lists.Iteration_Object;
   ...
   begin
      ...
      if Res.Server_Processing_Resource/=null and then 
   Res.Server_Processing_Resource.all in 
   Mast.Processing_Resources.Network.Network'Class
      then
         Ada.Text_IO.Put(File,"xsi:type=""mast2:Communication_Channel""");
         Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+4));
         Ada.Text_IO.Put_Line(File,"name=""" & 
            XMI.XMI_Name_Image(Res.Name, "com_") & """");
      else
         Ada.Text_IO.Put(File,"xsi:type=""mast2:Thread""");
         Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+4));
         Ada.Text_IO.Put_Line(File,"name=""" & 
            XMI.XMI_Name_Image(Res.Name, "th_") & """");
      end if;
      if Res.Server_Scheduler/=null then
         Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+4));
         Ada.Text_IO.Put_Line
           (File,"Scheduler=""" &
         XMI.XMI_Name_Image(Mast.Schedulers.Name(Res.Server_Scheduler), 
                        "sch_") &
         """>");
      end if;
      if Res.Server_Sched_Parameters/=null then
         Mast.Scheduling_Parameters.Print_XMI
           (File,Res.Server_Sched_Parameters.all,Indentation+2,False);
      end if;
      if Res.Server_Synch_Parameters/=null then
         Mast.Synchronization_Parameters.Print_XMI
           (File,Res.Server_Synch_Parameters.all,Indentation+2,False);
      end if;
      Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation));
      Ada.Text_IO.Put_Line(File,"</Element_List>");
   end Print_XMI;
   ...
   begin
      ...
      if (Sch.The_Sched_Params_Result/=null and then
        ...
        (Sch.The_Synch_Params_Result/=null and then
         Results.Synch_Params(Sch.The_Synch_Params_Result.all)/=null)
      then
         if Sch.Server_Processing_Resource/=null and then 
      Sch.Server_Processing_Resource.all in 
      Mast.Processing_Resources.Network.Network'Class
    then
            Is_Thread := False;
            Ada.Text_IO.Put
         (File,"xsi:type=""mast2_res:Communication_Channel_Result""");
         else
            Is_Thread := True;
            Ada.Text_IO.Put(File,"xsi:type=""mast2_res:Thread_Result""");
         end if;
         Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+2));
         Ada.Text_IO.Put_Line("<Model_Elem");
         Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+4));
         if Is_Thread then
            Ada.Text_IO.Put_Line
         (File,"<xsi:type=""mast2:Thread"""); 
            Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+4));
            Ada.Text_IO.Put_Line
         (File, "href=""" & 
       Var_Strings.To_String(Mast.XMI.Get_XMI_Model_File) &
       "#" & XMI.XMI_Name_Image(Sch.Name, "th_") & """/>");
         else
            Ada.Text_IO.Put_Line
         (File,"<xsi:type=""mast2:Communication_Channel""");   
            Ada.Text_IO.Set_Col(File,Ada.Text_IO.Count(Indentation+4));
            Ada.Text_IO.Put_Line
         (File, "href=""" & 
       Var_Strings.To_String(Mast.XMI.Get_XMI_Model_File) &
       "#" & XMI.XMI_Name_Image(Sch.Name, "com_") & """/>");
         end if;
         if Sch.The_Sched_Params_Result/=null and then
           Results.Sched_Params(Sch.The_Sched_Params_Result.all)/=null
         then
            Results.Print_XMI
              (File,Sch.The_Sched_Params_Result.all,Indentation+2,False);
         end if;
         if Sch.The_Synch_Params_Result/=null and then
           Results.Synch_Params(Sch.The_Synch_Params_Result.all)/=null
         then
   ...
   begin
      ...
      for I in 1..Lists.Size(The_List) loop
         Lists.Get_Next_Item(Sch_Ref,The_List,Iterator);
         Print_XMI_Results(File,Sch_Ref.all,Indentation,True);
      end loop;
   end Print_XMI_Results;
